...
class Tests:
    ...
    def run_checkpoint(self, checkpoint=1):
        logging.info("[CP%s] Executing tests for checkpoint %s." %
                     (checkpoint, checkpoint))
        self.cp_test_map[checkpoint-1]()
        logging.info("[CP%s] Tests for checkpoint %s complete. Verify that your results match the "
    ...
    def __test_parser_full_cp1(self):
        ...
        logging.info(
            "[CP1] Time taken to parse all records: %s second(s)" % self.pu.time_to_parse)
        logging.info("[CP1] Routes announced: %s | Routes withdrawn: %s" % (self.pu.n_announcements,
        ...
        while True:
            ...
            if next_updates['timestamp'] is None:
                logging.info(
                    "[CP1] No more updates to process in file: %s" % self.pu.filename)
                break
            else:
                logging.info("[CP1] At timestamp: %s | %s announcements | %s withdrawals" % (next_updates['timestamp'],
                                                                                             len(next_updates[
                                                                                                 'announcements']),
                                                                                             len(next_updates[
                                                                                                 'withdrawals'])))
    ...
    def __test_routing_applying_updates_cp2(self):
        ...
        while True:
            ...
            if next_updates['timestamp'] is None:
                logging.info(
                    "[CP2] No more updates to process in file: %s" % self.pu.filename)
                break
            else:
    ...
    def __test_hijacks_safe_updating_cp6(self):
        self.dh = DetectHijacks(start_table=self.rt,
                                monitored_range='208.65.153.0/21')
...
def main():
    ...
    parser.add_argument('--checkpoint', '-cp',
                        help="All code until the checkpoint ID will be executed.")
